...
files <- dir(here::here("input","contratos"), pattern = "contratos 2023_original.csv") # get file names
...
dt_contratos_clean <- dt_contratos_csv |>
  ...
  select(CODIGOCONVOCATORIA, ENTIDAD, RUCENTIDAD, ENTIDADCONTRATANTE, NUMITEM, MONTOREFERENCIALTOTAL, MONTOCONTRATADOTOTAL, MONTOCONTRATADOITEM, FECHASUSCRIPCIONCONTRATO,FECHAVIGENCIAINICIAL,FECHAVIGENCIAFINAL,FECHAVIGENCIAFINACTUALIZADA) |>
  rbind(dt_contratos_xls |>
          select(CODIGOCONVOCATORIA, ENTIDAD, RUCENTIDAD, ENTIDADCONTRATANTE, NUMITEM, MONTOREFERENCIALTOTAL, MONTOCONTRATADOTOTAL, MONTOCONTRATADOITEM, FECHASUSCRIPCIONCONTRATO,FECHAVIGENCIAINICIAL,FECHAVIGENCIAFINAL,FECHAVIGENCIAFINACTUALIZADA), fill = TRUE)

#### previo a union de convocatorio + contratos, crear indicadores trazadores #####
#### seguir con join convocatoria + contratos ####
...
result_flag <- dt_conv_busq |>
  ...
  left_join(dt_contratos_csv |>
              filter(red_flag == 1), by, relationship = "many-to-many") |>
